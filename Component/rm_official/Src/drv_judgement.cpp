...
void judgement_device::data_handler(uint8_t *p_frame) {
    ...
    switch (cmd_id) {
        ...
        case EVENT_DATA_ID:memcpy(&this->judge_rece_mesg.event_data, data_addr, data_length);
            break;
        case SUPPLY_ACTION_ID:memcpy(&this->judge_rece_mesg.supply_projectile_action_data, data_addr, data_length);
            if (this->judge_rece_mesg.supply_projectile_action_data.supply_robot_id ==
                this->judge_rece_mesg.game_robot_state_data.robot_id) {//如果是我在补弹
                this->SupplyingFlag = 1;
            } else {//如果不是我在补弹
                this->SupplyingFlag = 0;
            }
            break;
        ...
        case BUFF_MUSK_ID://机器人增益，状态改变后发送
            ...
            break;
        case AERIAL_ROBOT_ENERGY_ID:memcpy(&this->judge_rece_mesg.aerial_robot_energy_data, data_addr, data_length);
            break;
