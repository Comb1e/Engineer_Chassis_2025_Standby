...
__RAM_FUNC void Trajectory_Device::Update_Data()
{
    ...
    else if(ABS(this->target_count - this->already_count) < 0.05)
    {
...
__RAM_FUNC void Trajectory_Device::Change_Basic_Step(float new_step)
{
    ...
    this->target_count = (this->final - this->initial) / this->basic_step;
    this->already_count = (this->track_point - this->initial) / this->basic_step;
}
