...
//无功率环
__RAM_FUNC void Chassis_Motor_SolverSet(DJI_motor_t wheels[], float vel_x, float vel_y, float spin)
{
    float speeds[4];
    float speed_correction = 1;//这个是限制轮子的最大速度不是车体的速度,也可以直接在这里限制车速
    float w = 0.8f;
    float v;
    vel_x = ABS_Limit(vel_x, 1.0f);
    vel_y = ABS_Limit(vel_y, 1.0f);
    ...
    for(int i = 0; i < 4 ; i++)
    {//取最大值，这个比较合理，我之前是在speed出来之前进行分配，这个是出来之后进行分配，是一样的。
        if (fabsf(speeds[i]) > speed_correction)
    ...
    for (int i = 0; i < 4; i++)
    {
        if(wheels[i].state.reverse_flag)
        {
            wheels[i].target_data.target_vel = -speed_correction * speeds[i];
        }
        else
        {
            wheels[i].target_data.target_vel = speed_correction * speeds[i];
...
/**
  ...
  */
__RAM_FUNC void Super_Chassis_Motor_SolverSet(DJI_motor_t wheels[], float vel_x, float vel_y, float spin)
{
...
float pre_w;
__RAM_FUNC void Chassis_Motor_Position_SolverSet(DJI_motor_t wheels[], float x, float y,float w)//机械臂的坐标系和底盘坐标系xy相反
{
    float pos[4];
...
//    pos[3] = (x + y) / ECD_RATIO;                                        //define CHASSIS_MOTOR_RF    (0x204)
    ...
    for (int i = 0; i < 4; i++)
    {
        wheels[i].pid_loc.error += pos[i] - wheels[i].pid_loc.target_ecd;
    }
}