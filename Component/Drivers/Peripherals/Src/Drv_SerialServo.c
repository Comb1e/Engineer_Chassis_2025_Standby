...
{
    servo->huart = huart;
    servo->init_flag = true;
    servo->enable_flag = true;
    servo->id2dev[id]->ctrl_data.id = id;
    servo->id2dev[id]->state.servo_flag = true;
    servo->id2dev[id]->state.engine_flag = false;
    servo->id2dev[id]->state.lost_flag = true;
    servo->id2dev[id]->state.init_flag = true;
    servo->id2dev[id]->state.load_flag = true;
}
//要解决收发不能同时READ中可能要改
...
{
    taskENTER_CRITICAL();
    servo->id2dev[id]->ctrl_data.set_1000 = set;
    osMessageQueuePut(ServoCtrlQueueHandle, &servo->id2dev[id]->ctrl_data, 0, 0);
    taskEXIT_CRITICAL();
}
...
void LobotSerialServoMoveSet(servo_t *_servo, uint8_t _id, int16_t _position, uint16_t _time_ms) {
    ...
    _servo_dev->mode.current_pos = _position;
    _servo_dev->mode.current_ang = (float) _servo_dev->mode.current_pos * LOBOT_SERVO_POS2ANG;
}
...
void LobotSerialServoPreMoveSet(servo_t *_servo, uint8_t _id, int16_t _pre_pos, uint16_t _pre_time_ms) {
    ...
    _buf[9] = LobotCheckSum(_buf);
    LobotSerialWrite(_servo->huart, _buf, 10);
    _servo_dev->mode.pre_pos = (uint16_t) _pre_pos;
    _servo_dev->mode.pre_ang = (float) _servo_dev->mode.pre_pos * LOBOT_SERVO_POS2ANG;
    _servo_dev->mode.need_start_flag = true;
}
...
void LobotSerialServoStart(servo_t *_servo, uint8_t _id) {
    ...
    _servo_dev->mode.current_pos = (int16_t) _servo_dev->mode.pre_pos;
    _servo_dev->mode.current_ang = _servo_dev->mode.pre_ang;
    _servo_dev->mode.need_start_flag = false;
}
...
void LobotSerialServoSetID(servo_t *_servo, uint8_t oldID, uint8_t newID) {
    ...
    _servo_dev->ctrl_data.id = newID;
    _servo->id2dev[_servo_dev->ctrl_data.id] = _servo_dev;
}
...
void LobotSerialServoChangePosOffset(servo_t *_servo, uint8_t _id, int8_t _offset_pos) {
    ...
    _buf[6] = LobotCheckSum(_buf);
    LobotSerialWrite(_servo->huart, _buf, 7);
    _servo_dev->mode.pos_offset = _offset_pos;
    _servo_dev->mode.ang_offset = (float) _servo_dev->mode.pos_offset * LOBOT_SERVO_POS2ANG;
}
...
void LobotSerialServoPosLimit(servo_t *_servo, uint8_t _id, uint16_t _pos_min, uint16_t _pos_max) {
    ...
    if (_pos_min < _pos_max) {
        LobotSerialWrite(_servo->huart, _buf, 10);
        _servo_dev->mode.pos_max = _pos_max;
        _servo_dev->mode.pos_min = _pos_min;
    }
...
void LobotSerialServoSetEngineMode(servo_t *_servo, uint8_t _id, int16_t _speed) {
    ...
    if (_speed < -1000) _speed = -1000;
    if (_servo_dev->reverse_flag) _speed = -_speed;
    _buf[0] = _buf[1] = LOBOT_SERVO_FRAME_HEADER;
    _buf[2] = _id;
    _buf[3] = 7;
    ...
    _buf[9] = LobotCheckSum(_buf);
    LobotSerialWrite(_servo->huart, _buf, 10);
    _servo_dev->state.engine_flag = true;
    _servo_dev->state.servo_flag = false;
    _servo_dev->speedf = (float) _speed / 1000.0f;
}
...
void LobotSerialServoSetServoMode(servo_t *_servo, uint8_t _id) {
    ...
    _buf[9] = LobotCheckSum(_buf);
    LobotSerialWrite(_servo->huart, _buf, 10);
    _servo_dev->state.servo_flag = true;
    _servo_dev->state.engine_flag = false;
}
...
void LobotSerialServoSetUnload(servo_t *_servo, uint8_t _id) {
    ...
    _buf[6] = LobotCheckSum(_buf);
    LobotSerialWrite(_servo->huart, _buf, 7);
    _servo_dev->state.load_flag = false;
}
...
void LobotSerialServoSetLoad(servo_t *_servo, uint8_t _id) {
    ...
    _buf[6] = LobotCheckSum(_buf);
    LobotSerialWrite(_servo->huart, _buf, 7);
    _servo_dev->state.load_flag = true;
}
...
int16_t LobotSerialServoReadPosition(servo_t *_servo, uint8_t _id) {
    ...
    if (error_cnt < 30) {
        _servo_dev->mode.current_pos = (int16_t) MERGE_BYTES(rec_buf[6], rec_buf[5]);
        return _servo_dev->mode.current_pos;
    } else
        return INT16_ERROR_RETURN;
}
